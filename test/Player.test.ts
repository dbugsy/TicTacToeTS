...
describe("Player", () => {
  ...
  it("will not play if it is not their turn", () => {
    ...
    expect( () => player.play(currentPlayer)).toThrow(Player.OUT_OF_TURN_ERROR_MESSAGE);
  });
  ...
  it("will play if it is their turn", () => {
    ...
    expect( () => player.play(currentPlayer)).not.toThrow(Player.OUT_OF_TURN_ERROR_MESSAGE);
  });
});
